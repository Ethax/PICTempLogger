...
;Timer.c,69 :: 		PICTime* getSystemTime() {
...
;Timer.c,76 :: 		void setSystemTime(PICTime* time_ptr) {
...
;Timer.c,80 :: 		memcpy(&systemTime, time_ptr, sizeof(PICTime));
	...
	MOVF        FARG_setSystemTime_time_ptr+0, 0 
	MOVWF       FARG_memcpy_s1+0 
	MOVF        FARG_setSystemTime_time_ptr+1, 0 
...
L_end_setSystemTime:
	RETURN      0
; end of _setSystemTime

_timeToString:

;Timer.c,93 :: 		char* timeToString(PICTime* time_ptr) {
;Timer.c,95 :: 		uint8_t i = 0;
	CLRF        timeToString_i_L0+0 
;Timer.c,98 :: 		time_str[i++] = ((1900 + time_ptr->year) / 1000) + '0';
	MOVLW       timeToString_time_str_L0+0
	MOVWF       FLOC__timeToString+0 
	MOVLW       hi_addr(timeToString_time_str_L0+0)
	MOVWF       FLOC__timeToString+1 
	MOVF        timeToString_i_L0+0, 0 
	ADDWF       FLOC__timeToString+0, 1 
	BTFSC       STATUS+0, 0 
	INCF        FLOC__timeToString+1, 1 
	MOVLW       5
	ADDWF       FARG_timeToString_time_ptr+0, 0 
	MOVWF       FSR2 
	MOVLW       0
	ADDWFC      FARG_timeToString_time_ptr+1, 0 
	MOVWF       FSR2H 
	MOVLW       108
	MOVWF       R0 
	MOVLW       7
	...
	MOVLW       48
	ADDWF       R0, 1 
	MOVFF       FLOC__timeToString+0, FSR1
	MOVFF       FLOC__timeToString+1, FSR1H
	MOVF        R0, 0 
	MOVWF       POSTINC1+0 
	INCF        timeToString_i_L0+0, 1 
...
;Timer.c,125 :: 		time_str[i++] = (time_ptr->second / 10) + '0';
	...
	BTFSC       STATUS+0, 0 
	INCF        FLOC__timeToString+1, 1 
	MOVFF       FARG_timeToString_time_ptr+0, FSR0
	MOVFF       FARG_timeToString_time_ptr+1, FSR0H
	MOVF        POSTINC0+0, 0 
	MOVWF       R0 
	MOVLW       10
	MOVWF       R4 
